...
function sendPortalCommand(message) {
  ...
  return new Promise((resolve, reject) => {
    ...
    return openGlobalVendorPortal()
      .then(iframe => {
        let timeout = setTimeout(() => {
          delete portalCallMap[callId];
          reject(
            new Error(
              `${message.command} response not received from vendor domain within ${PORTAL_COMMAND_TIMEOUT_MILLISECONDS} milliseconds`
            )
          );
...
export { openGlobalVendorPortal, sendPortalCommand };
