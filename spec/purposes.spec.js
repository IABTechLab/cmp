...
describe("purposes page", () => {
	...
	describe("renders the purpose list", () => {
		it("lists standard purposes", () => {
			...
			expect(purposeList.getText()).toContain(
		...
		describe("interactions with the purpose list", () => {
			it("highlighted purposes have explanatory text", () => {
				...
				browser.wait(protractor.ExpectedConditions.presenceOf(el), 5000);
				expect(el.getText()).toContain(
					"The storage of information, or access to information that is already stored, on your device such as accessing advertising identifiers and/or other device identifiers, and/or using cookies or similar technologies."
				);
	...
	describe("purpose controls", () => {
		...
		it("clicking show companies expands the list of companies", () => {
			expect(element(by.css("[class*=purposes_vendorList]")).isPresent()).toBe(
				false
			);
