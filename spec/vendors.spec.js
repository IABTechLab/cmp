...
describe("vendors page", () => {
	beforeEach(() => {
		...
		const purposesVendorEl = element
			.all(by.css("[class*=purposes_vendorLink]"))
			.first();
		browser.wait(
			protractor.ExpectedConditions.presenceOf(purposesVendorEl),
			5000
		);
	...
	describe("vendors table", () => {
		it("renders", () => {
			...
			browser.wait(
				protractor.ExpectedConditions.presenceOf(vendorContent),
				5000
			);
			const table = vendorContent.element(
				by.css("[class*=vendors_vendorList]")
			);
